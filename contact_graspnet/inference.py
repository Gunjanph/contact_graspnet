...
def inference(global_config, checkpoint_dir, input_paths, K=None, local_regions=True, skip_border_objects=False, filter_grasps=True, segmap_id=None, z_range=[0.2,1.8], forward_passes=1):
    ...
    for p in glob.glob(input_paths):
        ...
        pred_grasps_cam, scores, contact_pts, gripper_openings, gripper_width = grasp_estimator.predict_scene_grasps(sess, pc_full, pc_segments=pc_segments, 
                                                                                          local_regions=local_regions, filter_grasps=filter_grasps, forward_passes=forward_passes)  
        
        # print(pred_grasps_cam.shape, scores.shape)
        # print(pred_grasps_cam, scores)

        # Save results
        np.savez('results/predictions_{}'.format(os.path.basename(p.replace('png','npz').replace('npy','npz'))), 
                  pred_grasps_cam=pred_grasps_cam, scores=scores, contact_pts=contact_pts, gripper_openings=gripper_openings, gripper_width=gripper_width)
        # print(pred_grasps_cam, end=" ")
        # print(scores, end=" ")
        # Visualize results          
...
if __name__ == "__main__":
    ...
    parser.add_argument('--flag', type=int, default=0, help='type 0 if want to generate grasp, 1 if want to visualise')
